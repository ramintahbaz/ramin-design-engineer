...
function Stack({ exploded, useRealSpring, label }: {
  exploded: boolean; useRealSpring: boolean; label: string;
}) {
  return (
...
export default function PhotoBoomSpringDemo() {
  const [exploded, setExploded] = useState(false);

  return (
    <div className="w-full flex flex-col items-center gap-8 py-10 px-6 rounded-xl bg-neutral-900">
      <div className="flex flex-col items-center gap-12 w-full">
        <Stack exploded={exploded} useRealSpring={true} label="Spring physics" />
        <div style={{ height: 1, width: 120, background: 'rgba(0,0,0,0.08)' }} />
        <Stack exploded={exploded} useRealSpring={false} label="Linear easeOut" />
      </div>
      <button
